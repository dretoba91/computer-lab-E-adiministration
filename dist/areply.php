<?php
    ...
    include("aheader.php");
        ...
        //    var_dump($reply);
           ...
           $sql = "INSERT INTO reply (fid, uname, comment, departments, reply) VALUES ('$fid', '$name', '$comment', '$department', '$reply')";
        //    var_dump($sql);
        //    die();
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <form method="POST" action="<?php $_PHP_SELF ?>" name="reply">
                        
                        <?php 

                           

                            // Button click to reply using POST method

                            if(!empty($_GET['id'])) {
                                $id = $_GET['id'];

                                    // Fetch record with ID and populate it in the form
                                    $query2 = "SELECT * FROM feedback WHERE fid ='".$_GET['id']."' ";
                                    // var_dump($query2);
                                    // die();

                                    // To execute the select the query
                                    if(!($selectedResult = mysqli_query($conn, $query2))) {
                          ...
                          <?php 
                              ...
                              if(mysqli_num_rows($selectedResult) == 0) {
                                  ...
                                  while ($row = mysqli_fetch_assoc($selectedResult)) {
                                      echo "<tr>";
                                      echo '<td><input type="text" name="fid" value=" ' . $row['fid'] . '" required></td>';
                                      echo '<td><input type="text" name="uname" value=" ' . $row['uname'] . '" required></td>';
                                      echo '<td><input type="text" name="comment" value=" ' . $row['comment'] . '" required></td>';
                                      echo '<td><input type="text" name="departments" value=" ' . $row['departments'] . '" required></td>';
                                      echo '<td><textarea name="c_reply" id="" cols="15" rows="5"></textarea></td>';
                          ...
                          ?>  
                            ...
                            <td width = "250"> </td>
                                    <td><input name = "reply" type = "submit" value = "Reply Report"></td>
                        </form>
                        <a href="afeedback.php" class="head4">
